...
The service validates JWT tokens signed by the user-service using RSA-256 asymmetric keys. Tokens must be included in the `Authorization` header:

```
Authorization: Bearer <token>
```

The service extracts `user_id` from the JWT payload for ownership tracking.
